...
with minutely_data as (
    select
        plant_id,
        fruit_type,
        timestamp_trunc(measured_at, day) as date_utc,
        timestamp_trunc(measured_at_localtime, day) as date_localtime,
        temp_room_c,
        temp_pulp_c,
        rh_room_pct,
        co2_ppm,
...
hourly_aggregation as (
    select
        ...
        fruit_type,
        date_utc,
        min(date_localtime) as date_localtime,
        ...
        safe_divide(
            ...
            count(compressor_on)
        ) as compressor_duty_cycle_daily,
        safe_divide(
            countif(humidifier_on = 1),
            count(humidifier_on)
        ) as humidifier_duty_cycle_daily,
        safe_divide(
            sum(q_total_w),
